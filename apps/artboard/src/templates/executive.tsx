    ...
    <div className="mb-6 border-b border-blue-900 pb-4">
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="text-3xl font-light tracking-tight text-blue-900">{basics.name}</div>
          {basics.headline && (
...
const Summary = () => {
  ...
  return (
    <section id={section.id} className="mb-5">
      <h3 className="mb-3 text-base font-bold uppercase tracking-wider text-blue-900 border-b border-blue-900 pb-1 font-serif">
        {section.name}
      </h3>
      <div
...
const Rating = ({ level }: RatingProps) => (
  <div className="relative h-1 w-24 bg-gray-200">
    <div
      className="absolute inset-0 h-1 bg-blue-900"
    ...
    <Section<Experience> section={section} urlKey="url" summaryKey="summary">
      {(item) => (
        <div>
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <div className="font-semibold text-blue-900">{item.position}</div>
    ...
    <Section<Education> section={section} urlKey="url" summaryKey="summary">
      {(item) => (
        <div>
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <div className="font-semibold text-blue-900">{item.area}</div>
              <div className="text-sm text-gray-700">
                {isUrl(item.url.href) ? (
                  <a href={item.url.href} target="_blank" rel="noreferrer noopener nofollow" className="hover:underline">
                    {item.institution}
...
export const Executive = ({ columns, isFirstPage = false }: TemplateProps) => {
  const [main, sidebar] = columns;

  return (
    <div className="space-y-4 p-custom font-serif">
      {isFirstPage && <Header />}

      <div className="grid grid-cols-3 gap-x-6">
        <div className="main col-span-2 space-y-5">
          {main.map((section) => (
        ...
        {sidebar.length > 0 && (
          <div className="sidebar space-y-5 border-l border-gray-300 pl-4">
            {sidebar.map((section) => (
              <Fragment key={section}>{mapSectionToComponent(section)}</Fragment>
            ))}
          </div>
